...
<body id="empresa">
	...
	<?php
		$id = $_GET["id"];
		$query2 = mysql_query("SELECT * FROM products WHERE id=$id",$link);
		$result2 = mysql_fetch_array($query2, MYSQL_ASSOC);

		//$query2 = mysql_query("SELECT * FROM categories WHERE id=$id",$link);
		//$result2 = mysql_fetch_array($query2, MYSQL_ASSOC);
	?>
	...
	<section style="overflow: hidden;">
		...
		<div class="columnaIzquierda">
			...
			<?php 
				$query = mysql_query("SELECT * FROM products order by nombre",$link);
				if( mysql_num_rows($query) > 0 ){
					...
					while ($result = mysql_fetch_array($query, MYSQL_ASSOC)){
						?>
							<li>
								<?=$result["nombre"];?> 
								<a href="./productos_edit.php?id=<?=$result['id'];?>">Editar</a> 
								<a class="delete" href="./productos_delete.php?id=<?=$result['id'];?>">Borrar</a>
							</li>
						<?php
					}
					?>
		...
		<div class="columnaDerecha">
			...
			<?php 
				$queryCateg = mysql_query("SELECT * FROM categories order by nombre",$link);
				if( mysql_num_rows($queryCateg) > 0 ){
			?>

			<form action="./productos_edit_post.php" enctype='multipart/form-data' method="post">
				<fieldset>
					<input type="hidden" value="<?= $id; ?>" name="id" id="id"/>
					<p><label for="nombre">Nombre:</label> <input type="text" id="nombre" name="nombre" class="text" value="<?= $result2['nombre']; ?>" /></p>
					
					<p><label for="descripcion">Descripcion:</label><br />
					<textarea id="descripcion" name="descripcion"><?= $result2['descripcion']; ?></textarea></p>
					
					<p><label for="categoria">Categoria:</label>
					<select id="categoria" name="categoria">
						<?php 
							while ($resultCategs = mysql_fetch_array($queryCateg, MYSQL_ASSOC)){
						?>
							<option value="<?= $resultCategs['id']?>" <?php if($resultCategs['id'] == $result2['categ_id']) echo "selected"; ?>>
								<?= $resultCategs["nombre"]?>
							</option>
						<?php
							}
					...
					<p>Foto del producto actual:<br>
					<img src="../fotos-productos/<?=$result2['imagen'];?>" width="200" />
					</p>

					<p><label for="foto">Quer&eacute;s cambiar la foto? seleccionala:</label>
			...
			<?php }else{ ?>
				<p>Deb&eacute;s tener al menos una categoria creada. <a href="categorias.php">Crear categoria</a></p>
			<?php } ?>
		</div>
	...
	tinyMCE.init({
        ...
        // Theme options
        theme_advanced_buttons1 : "bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,code,removeformat",
        ...
        inline_styles : false,
        invalid_elements : "h1,h2,h3,h4,h5,h6,div,span",
