...
;(function(){
	...
	// Variables publicas
		...
		navegacionAsync.initial_section = $("section.contenido:first");
		...
		// Flag para saber si entender si ejecuto o no el scroll
		...
		var load_sections = function(){
		    ...
		    navegacionAsync.nav.find("a").each(function(){
		        ...
		        // Si no tiene la class active entro
			        ...
			        // Llamo a la seccion, solo lo que esta dentro del class .contenido
			        section_wrapper.load(section_to_load + ' .contenido', function(response,status, xhr){
	...
	// Metodos publicos
		...
		navegacionAsync.set_active = function(page_id){
			...
			navegacionAsync.nav.find("a").removeClass("active");

			var link_to_active = navegacionAsync.nav.find("a[data-nav\\:id="+page_id+"]");

			link_to_active.addClass("active");
		...
		navegacionAsync.scrollTo = function(link,notDoPushState){
			...
			if( page_id != "contacto"){ $("footer").removeClass("show"); }
			...
			$.scrollTo("#" + page_id, navegacionAsync.speed_scroll, {
				...
				onAfter: function(){
					...
					if( page_id == "contacto"){	$("footer").addClass("show"); }
			...
			navegacionAsync.nav.stop(true,true).fadeOut(100,function(){

				navegacionAsync.nav.delay(600).fadeIn(600);
		...
		navegacionAsync.init = function(){
			...
			if( navegacionAsync.initial_section_id == "contacto"){	$("footer").addClass("show"); }
		}
