;(function(){
	...
	$('#envio-consulta').on('submit', function(event){
		...
		var empresaIngresado = $('#empresa').val();
		...
		if( error ){
			...
			$.ajax({
				type: 'POST',
				data: 'name='+nameIngresado+'&email='+emailIngresado+'&telefono='+telefonoIngresado+'&empresa='+empresaIngresado+'&message='+messageIngresado,
				dataType: 'json',
				...
				success: function(r) {
					...
					if(r.status == 'ok') {
						$('#nombre, #email, #empresa, #telefono, #consulta').val('');
